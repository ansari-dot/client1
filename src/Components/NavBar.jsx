...
const NavBar = () => {
  ...
  return (
    <>
      {/* Navbar */}
      <nav
        className={`fixed top-[35px]  left-0 right-0 md:[40px] z-50 flex items-center justify-center px-6 py-2 whitespace-nowrap transition-all duration-300 ${isScrolled ? 'bg-white border-b border-gray-200' : 'backdrop-blur-md bg-white/10 border-b border-white/20'}`}
      >
        ...
        <button
          className={`md:hidden absolute top-[35px] right-6 text-2xl transition-colors ${(isScrolled || isDigital) ? 'text-gray-700' : 'text-white' }`}
        ...
        <ul className="flex flex-col mt-16 px-6 font-semibold gap-4">
          {["Home", "About", "Contact","Career"].map((item, idx) => (
            <li key={idx}>
              <Link
                to={item === "Home" ? "/" : `/${item.toLowerCase()}`}
                className="block text-gray-800 hover:text-[#15487d] transition text-lg"
                onClick={() => setMenuOpen(false)}
              >
                {item}
              </Link>
