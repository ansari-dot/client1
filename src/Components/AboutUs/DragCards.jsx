...
export const DragCards = () => {
  return (
    <section className="relative grid min-h-screen w-full place-content-center overflow-hidden bg-gray-200 rounded-4xl">
      <h2 className="relative z-0 text-[4vw] font-black text-white md:text-[80px]">
        Webmentix<span className="text-indigo-500">.</span>
      </h2>
      <Cards />
    </section>
  );
...
const Card = ({ containerRef, src, alt, top, left, rotate, className }) => {
  ...
  return (
    <motion.img
      ...
      dragElastic={0.65}
      className={`drag-elements absolute bg-neutral-200 p-1 pb-4 rounded-xl shadow-lg cursor-grab active:cursor-grabbing ${className}`}
      // Scroll animation (fade in + pop effect)
      initial={{ opacity: 0, scale: 0.8, y: 50 }}
      whileInView={{ opacity: 1, scale: 1, y: 0 }}
      viewport={{ once: true, amount: 0.3 }}
      transition={{ duration: 0.6 }}
      // Hover animation (3D tilt + zoom)
      whileHover={{ scale: 1.1, rotateZ: 2, rotateY: 10 }}
      whileTap={{ scale: 0.95 }}
    />
  );
};
